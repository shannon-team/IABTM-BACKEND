...
optimizedUserSchema.index({ isOnline: 1, lastSeen: -1 }); // Online users
optimizedUserSchema.index({ role: 1, isOnline: 1 }); // Online experts/admins
optimizedUserSchema.index({ email: 1, emailVerified: 1 }); // Email verification
optimizedUserSchema.index({ 'subscription.plan': 1, 'subscription.status': 1 }); // Subscription queries
optimizedUserSchema.index({ friends: 1, isOnline: 1 }); // Friends online status
optimizedUserSchema.index({ createdAt: -1, role: 1 }); // User registration analytics

// Text search index for user discovery
optimizedUserSchema.index({
    name: 'text',
    profileName: 'text',
...
optimizedUserSchema.pre('save', async function (next) {
...
optimizedUserSchema.methods.comparePassword = async function(candidatePassword) {
    return bcrypt.compare(candidatePassword, this.password);
};

// Instance method to update presence
optimizedUserSchema.methods.updatePresence = async function(status = 'online', roomId = null) {
...
optimizedUserSchema.statics.getOnlineUsers = function(limit = 100) {
    return this.find({
        isOnline: true,
...
optimizedUserSchema.statics.getUsersByRole = function(role, limit = 50) {
...
optimizedUserSchema.virtual('fullName').get(function() {
    return this.name || this.profileName || 'Anonymous User';
});

// Ensure virtuals are included in JSON
optimizedUserSchema.set('toJSON', { virtuals: true });
optimizedUserSchema.set('toObject', { virtuals: true });

const OptimizedUser = mongoose.model('OptimizedUser', optimizedUserSchema);

export default OptimizedUser; 